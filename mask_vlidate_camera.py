...
while True:
    ...
    color_img = cv2.resize(color_img, dim, interpolation=cv2.INTER_AREA) # カラー画像をリサイズ
    ...
    for (x,y,w,h) in faces: #　顔を1枚づつ取り出す
        ...
        cv2.imwrite('input/%d%dface.jpg'%(img_count_total, img_count), color_face) # ファイルに書き出す
        img = load_img('input/%d%dface.jpg'%(img_count_total, img_count),target_size=(img_width, img_height)) # ロードしてリサイズ
        ...
        cv2.rectangle(color_img,(x,y), (x+w, y+h), (0,0,255), 3) # 顔を囲む矩形を描画. Red(BGR)
        cv2.putText(color_img, class_label, org, font, fontScale, color, thickness, cv2.LINE_AA) # テキストラベル
